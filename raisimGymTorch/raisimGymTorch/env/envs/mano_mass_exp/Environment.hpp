...
namespace raisim {
    ...
    public:
        ...
        explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
            ...
            /// add table: an invisible (alpha 0) 2x1x0.5m collision box centered at
            /// (1.25, 0, 0.25), so its top face sits at z=0.5, giving the object its
            /// resting support. switch_table_support(true) teleports this box far below
            /// the scene (out of collision range) to remove that support mid-episode;
            /// switch_table_support(false) teleports it back to (1.25, 0, 0.25).
...
//                leg4->setPosition(2.2275,-0.4875,0.2375);
                ...
                aff_center_visual[5] = server_->addVisualSphere(body_parts_r_[5]+"_aff_center", 0.02, 1, 0, 0, 1); // hand's current point: red
        ...
        void reset() final {
            if (first_reset_)
            {
                ...
                /// restore the table to its supporting pose in case switch_table_support
                /// removed it during the previous episode
        ...
        float* step(const Eigen::Ref<EigenVec>& action_r, const Eigen::Ref<EigenVec>& action_l) final {
            ...
            if (visualizable_){
                raisim::Vec<3> obj_com_w = arctic->getCOM();
                com_marker_->setPosition(obj_com_w.e());
            }
        ...
        /// Toggle the table's physical support for the object mid-episode. Called from
        /// runner.py's --no-support flag at the same grasp->lift transition step as
        /// switch_root_guidance, i.e. once the hand has a firm grasp, right before
        /// lift-off. is_on=true teleports the (invisible) table collision box far below
        /// the scene so it can no longer hold the object up; is_on=false teleports it
        /// back to its normal resting pose under the object. Both reset() and
        /// reset_state() unconditionally restore the table's resting pose and
        /// support_removed_ at the start of every rollout, so this only ever affects the
        /// remainder of the current episode.
        void switch_table_support(bool is_on) {
            support_removed_ = is_on;
            if (is_on) {
                box->clearExternalForcesAndTorques();
                box->setPosition(1.25, 0, -10.0);
                box->setVelocity(0,0,0,0,0,0);
            } else {
                box->clearExternalForcesAndTorques();
                box->setPosition(1.25, 0, 0.25);
